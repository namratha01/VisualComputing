...
def cumulative_map(energy_map):
        ...
        for row in range(1, m):
            for col in range(n):
                if col == 0:
                    e_right = output[row - 1, col + 1]
                    e_up = output[row - 1, col]
                    output[row, col] = energy_map[row, col] + min(e_right, e_up)
                elif col == n - 1:
                    e_left = output[row - 1, col - 1]
                    e_up = output[row - 1, col]
...
def find_seam(cumulative_map):
        ...
        output = numpy.zeros((m,), dtype=numpy.uint32)
        output[-1] = numpy.argmin(cumulative_map[-1])
        for row in range(m - 2, -1, -1):
            prv_x = output[row + 1]
            if prv_x == 0:
                output[row] = numpy.argmin(cumulative_map[row, : 2])
            else:
                output[row] = numpy.argmin(cumulative_map[row, prv_x - 1: min(prv_x + 2, n - 1)]) + prv_x - 1
        return output

for intRemove in tqdm.tqdm(range(100)):
	intSeam = []
	...
	for intY in range(1, len(intSeam)):
		assert(int(intSeam[intY]) - int(intSeam[intY - 1]) in [-1, 0, 1])
	# end
